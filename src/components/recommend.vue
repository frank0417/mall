    ...
    <aside class="recommend_detail">
      ...
      <aside class="recommend_buttom">
       ...
       <aside class="recommend_right recommend_right_margin">
        <aside class="recommend_time">
          <span v-if="state==2 && marketable">{{startTime.substring(11,16)}}-{{endTime.substring(11,16)}}</span>
          <span v-else-if="state==1 && marketable">{{startTime.substring(11,16)}} 开抢</span>
          <span v-else-if="state==0"></span>
        </aside>
...
<script>
  export default {
        ...
        name: "recommend",
        filters:{
          btnFilter(state,marketable){
            if(state =='2' && marketable){
              return '马上抢';
            }else if(state =='1' && marketable){
              return '即将开始';
            }else{
              return '抢光了';
            }
